...
## Read the build's own exit code, and reinstall before believing it

2026-09-23 — Two ways a local `npm run build` gets misread as green. First, `npm run build | grep ... | head -3` reports **`head`'s** exit code, not the build's, so a failed build prints nothing and looks like a clean pass — the pattern matched no error line precisely *because* the build died. Redirect to a file, echo `$?`, and read the log. Second, after a rebase or pull that moved `package.json`, the build fails with `Module not found` in a file you never touched (`three`, `libheif-js`): a declared dependency simply is not in `node_modules` yet. **How to apply**: on a fresh container or after any rebase, `npm install` before building, and treat a module-not-found in unfamiliar code as a stale install rather than a broken commit. The same applies to `npm run check` — its three steps are chained with `&&`, so the exit code is honest there, but piping it through `grep -v warning` hides the `✖ N problems` summary line that says whether any were errors.
...
## A shader change is measured and pixel-diffed, not eyeballed

2026-09-22 — `scripts/bench-sketch.mjs` renders deterministic frames of a sketch through the engine's headless-capture controller (`window.__sketchCapture`, the recorder's own path: the recording clock is pinned to the frame index, so frame *k* is the same pixels in every build), reports ms per frame with the lit fraction, dumps the pixels, and `--diff`s two dumps. It fails closed on any console or page error, because a GLSL compile failure is only a `console.error` after which the canvas stays black — a broken shader would otherwise bench as the fastest build ever. **How to apply**: dump before, change, rebuild, dump after, diff; assert a bounded difference for a GPU-side change (never byte equality, see `sketches.md`) and quote the before/after ms in the commit body. SwiftShader numbers are ratios — it prices a texture fetch far above a real GPU — and `--gpu` is for a machine that has one. Recipe and gotchas: `.claude/skills/verify/SKILL.md`, `scripts/README.md`.
