...
## Per-beat overrides sit beside the shared value, never replace it

2026-08-26 — `photo/photo-stamp-collage` can give each photo its own stamp position (`stamp.perImage` + `stamp.positions`), and the shape it uses is the one to copy for any "one value per photo" option on a sketch whose images come from an `images-stack`. The shared `stamp.position` stays the default and the per-beat list only overrides it, so flipping the switch on changes nothing until a beat is actually moved, and a preset saved before the option existed keeps rendering as it did. The list is indexed by **beat**, not by image — the sketch's `pair` mode consumes two images per beat, and indexing by image would silently halve the mapping there. On a canvas click the whole array is rewritten (`mergeChangedInPlace` replaces arrays wholesale, it does not merge them element by element) with every hole filled by what that beat was already showing, so writing one entry never moves the others and the form always shows as many rows as there are beats — which is what makes the click affordance usable without the user first adding rows by hand. **How to apply**: the draw loop must publish its beat index into the click-handler state (`sketchState.lastFrame`), or a click lands on whatever beat the defaults imply rather than the one on screen. An alternative was rejected: converting `images` to an `item-list` of `{ photo, position }` pairs, as `photo/photo-zoom-point` does. It is cleaner for a *new* sketch, but it breaks every stored preset and the `/embed` URLs that carry `images` — so a sketch that already ships with `images-stack` gets a parallel list instead.
