...
- **`first:`/`last:` variants cannot pick out a group.** Every `FieldRenderer` return is wrapped in its own `<div>`, so each group is always `:first-child` — a `first:border-t-0` silently removed *every* rule (`borderTopWidth` measured `0px` on all of them). The parent form decides instead: `GenericObjectForm` passes `showTopRule={index > 0}`. Assume the same about any positional variant inside this form tree.
