...
- 2026-09-23 — Most noise-driven p5 sketches never call `noiseSeed()` (~77 call `p.noise`, ~9 seed it), so they draw a different field on every page load: a backend recording or a shared `/embed` does not match the preview it came from (in-page capture is consistent, which hides it). Seeded `random()`/`noise()` agree across p5 1.x and 2.x; only the unseeded table is per-load. An engine-level `noiseSeed` in `sketch.js` would fix all at once but changes how each affected sketch looks. Also open: `photo/dominant-colors-block-switch` throws on a failed image (`null.width`) and never reaches its first frame, on 1.x and 2.x alike.
