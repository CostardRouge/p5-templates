...
- 2026-09-01 — **Loading-screen UX, agreed with the maintainer, not yet built.** The asset-loading telemetry added in PR #270 shifts the sketch-page poster (centred `flex-col`, so any line it adds re-centres the stack) and its counter climbs (`total` is steps-opened-so-far; fonts open only when sketch code touches a `string.fonts.*` getter mid-draw). Agreed: precompute the total from options (2 modules + declared images) before any step opens, clamp displayed progress **monotonic** so late font steps decelerate it instead of making it jump back, and mount no loading screen at all under ~150 ms so instant/cached loads never flash. Adding a `fonts` declaration to the `Assets` schema was rejected as out of scope (77 sketches call the getters from code). Four layouts were pitched as an artifact — poster-as-bar, hairline track + reserved caption, floating strip, corner ring — all holding the poster still via out-of-flow positioning or reserved fixed height; the maintainer picks one. **Trap found while designing**: `cache` (`src/sketches/p5/utils/cache.js`) is a module singleton that is never cleared, so revisiting a sketch with warm `imagesMap` opens *no* image steps — a naively precomputed total then overshoots and the bar stalls mid-way. Reconcile the plan in `options.js` (the only place that knows the cache) and force 100% before `ready`.
