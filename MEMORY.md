...
- A sketch exposes what it derives inside `draw()` through `probe( name, value )` — the third HUD source family (`probe:<name>`), the only one that is not an input, capture-safe by construction; slots are per instance and per frame with no allocation, a name written N times a frame is a fold (`probe.fold`), and the studio discovers probes by running the sketch, never from a declaration → `probes.md`, `sketches.md`.
...
- 2026-09-23 — **Probes, phase 1 + inspector shipped** on one sketch (`dragon-corridor-v3-grid`). Open, in order (`docs/probe-system.md` §7): a layer's probes are recorded but not addressable (`probe:<layerId>.<name>` needs a stable layer identity — see how the `sketch` content item identifies itself); probes as binding channels (phase 2: merge into `sampleChannels`, per-binding input range with a "learn", the one-frame delay noted in the popover); `probe.each` for a per-element stagger (phase 3, waits for a sketch). Every sketch touched from now on gets its probes (`sketches.md`).
...
| `docs/memory/probes.md` | Exposing a value a sketch computes inside `draw()`: `probe()` / `probe.fold`, the per-instance registry, the `probe:` HUD source, the live picker group, the Probes inspector card |
