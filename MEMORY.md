...
- A `sculpt` sketch decides on the CPU (field, order, envelope, links — unit-tested) and its shader only reads two data textures; a letter's readability is the rasterised-ink mask plus the "link midpoint on the ink" rule, never the outline chain → `sketches.md`.
...
- 2026-09-21 — `sculpt-v1-letter-relief` ships without a thumbnail or preview (captured from the studio, like `flip`). The category is meant to grow: the sphere variant (points on and inside a sphere, waves radial / axial) is being built in a parallel session on the same `_shared.js` and camera rig, and the two cannot both be `v1` — settle the numbers before the second one merges, the only moment a rename is free. Not built in v1, by choice: a 12-cell neighbourhood (needs three more link channels), a `both` mask mode, a residual wobble, rings-v10's virtual cursors.
