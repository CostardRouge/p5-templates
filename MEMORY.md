...
- A sketch declares which physical control drives a field (`binding: { control }`); the map is keyed on the MIDI PORT name, it resolves rather than mints channels, and a declared binding is rebuilt every frame instead of ever being written to the document → `midi-controllers.md`.
- A pad is a scalar channel (`midi.note<n>`, 0 after release), a button is ONE ordinary write (`component: "button"`, `effect` on a target — never a callback into the sketch), and a pad pressing it is an editor event that never enters the binding resolver → `midi-controllers.md`, `sketches.md`.
- The engine owns the MIDI output: paired with the input by port name, armed when the map says so, and ALWAYS released (pads off, DAW mode off) on reset, dispose, device switch and `pagehide`; LED wishes cross by abstract control through `padLedBridge` and only the diff is sent → `midi-controllers.md`.
...
- 2026-09-23 — `midi/pad-rack-v1-launchkey` (hidden from home) is the pad rig's demonstration and ships without thumbnail or preview, like the `flip` cascades. The whole pad path — channels, buttons, LEDs, arming — is verified by tests and build only: the hardware pass (DAW port picked → top row lit with the pressed option orange; a pad press moves it; a bottom pad toggles its layer; closing the tab darkens and disarms) is the maintainer's, and the MIDI port's LEDs are unmeasured.
...
| `docs/memory/midi-controllers.md` | A physical controller: `binding.control`, the port-keyed map, pads, buttons a pad presses, LEDs, arming the DAW port |
