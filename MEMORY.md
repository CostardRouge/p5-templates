...
- 2026-09-21 — Open in `sculpt`: three sketches now, sharing only the camera rig. A scattered lattice, a sheet of heights and a mesh of links are different data, so nothing is folded together until a fourth variant actually wants one of them — and each module is named for its content, never for the shape it draws, because two sessions independently reached for `_sheet.js`. Left out of `sculpt-v2-letter-relief` by choice: a 12-cell neighbourhood (three more link channels), a `both` mask mode, a residual wobble on raised points, rings-v10's virtual cursors.
