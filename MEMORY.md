...
- A sculpt can be a grid instead of a scattered lattice, and then its links are implicit (`_mesh.js`): the whole field travels as two data textures the shader addresses per cell, its logic is decided on the CPU, and a letter's readability is the rasterised-ink mask plus the "link midpoint on the ink" rule, never an outline chain → `sketches.md`.
