...
export function useProgress(initialSteps: Omit<ProgressStep, 'status'>[]) {
  ...
  const completeStep = useCallback((stepId: string) => {
    setSteps(prev =>
      prev.map(step =>
        step.id === stepId
          ? { ...step, status: 'completed' as const, percentage: 100 }
          : step
      )
    );
  }, []);

  const updateStepProgress = useCallback((stepId: string, percentage: number) => {
    setSteps(prev =>
      prev.map(step =>
        step.id === stepId ? { ...step, percentage } : step
      )
    );
  }, []);

  const errorStep = useCallback((stepId: string) => {
    setSteps(prev =>
      prev.map(step =>
  ...
  const reset = useCallback(() => {
    setSteps(prev =>
      prev.map(step => ({ ...step, status: 'pending' as const, percentage: 0 }))
    );
