...
class Spiral extends SpiralBase {
  draw(
    ...
    const waveAmplitude = size / ( spiralOpts.waveAmplitudeDivisor ?? 3.5 );
    ...
    const cadenceIndexScale = motion.cadenceIndexScale ?? 0;
    const cadenceMin = spiralOpts.cadenceMin ?? -4;
    const cadenceMax = spiralOpts.cadenceMax ?? 4;
    const circleSize = spiralOpts.circleSize ?? 200;
