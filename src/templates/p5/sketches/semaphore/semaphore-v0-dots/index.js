...
class SemaphoreSpiral extends SemaphoreBase {
  draw(
    time, index
  ) {
    const p = getP5();
    ...
    const shadowOffsetDeg = spiralOpts.shadowOffsetDeg ?? 5;
    ...
    const fillColor = colorOpts.fillColor ?? [
      0
    ];
...
sketch.setup( () => {
  rebuildGrid( {
    ...
    SemaphoreClass: SemaphoreSpiral
  } );
} );
