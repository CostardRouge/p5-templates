...
sketch.draw( (
  ...
  const gridColumns = options.sketch.grid?.columns ?? 3;
  const gridRows = options.sketch.grid?.rows ?? 3;
  ...
  const marginTop = options.sketch.path?.marginTop ?? 50;
  ...
  const branchTimeOffsetSign = options.sketch.branches?.timeSign ?? 1;
  const branchPhaseDivider = options.sketch.branches?.phaseDivider ?? 40;

  const waveStrengthEasing = easing?.[ options.sketch.waves?.strengthEasing ] ?? easing.easeInSine;
  const waveCountEasing = easing?.[ options.sketch.waves?.countEasing ] ?? easing.easeOutQuart;
  ...
  const hueIndexEasing = easing?.[ options.sketch.colors?.hueIndexEasing ] ?? ( ( x ) => x );
  ...
  const branchTurns = Math.round( branchTimeOffsetSign );
