...
class Spiral extends SpiralBase {
  draw(
    ...
    const shadowsCountSpeed = spiralOpts.shadowsCountSpeed ?? 3;
    const shadowIndexStep = spiralOpts.shadowIndexStep ?? 0.1;
    const linesCountMin = spiralOpts.linesCountMin ?? 1;
    const linesCountMax = spiralOpts.linesCountMax ?? 7;
    const sizeRatio = spiralOpts.sizeRatio ?? 1;
    const strokeWeightFactor = spiralOpts.strokeWeightFactor ?? 0.25;
    ...
    const shadowsCountSpeedTurns = Math.round( shadowsCountSpeed );
    ...
    const shadowsCount = p.map(
      p.cos( index + time * shadowsCountSpeedTurns ),
      -1,
      1,
      shadowsCountMin,
      shadowsCountMax,
