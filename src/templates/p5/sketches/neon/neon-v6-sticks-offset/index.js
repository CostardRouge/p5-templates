...
class Spiral extends SpiralBase {
  ...
  draw(
    ...
    const opacityMax = spiralOpts.opacityMax ?? 7;
    const angleSpokes = spiralOpts.angleSpokes ?? 10;
    const offsetDegrees = motion.offsetDegrees ?? 20;
    const ringExpandMin = spiralOpts.ringExpandMin ?? 0;
    const ringExpandMax = spiralOpts.ringExpandMax ?? 1;
    ...
    for ( let shadowIndex = 0; shadowIndex <= shadowsCount; shadowIndex++ ) {
      ...
      const s = p.map(
        p.cos( t + p.radians( shadowIndex * offsetDegrees ) ),
        -1,
        1,
        size * ringExpandMin,
        size * ringExpandMax
