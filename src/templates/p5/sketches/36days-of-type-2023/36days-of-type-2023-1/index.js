...
sketch.draw( () => {
  ...
  const text = options.sketch?.shape?.text ?? "1";
  const size = ( options.sketch?.shape?.size ?? 0.75 ) * p.width;
  ...
  const depthMin = options.sketch?.shape?.depthMin ?? 20;
  const depthMax = options.sketch?.shape?.depthMax ?? 500;
  const fillAlpha = options.sketch?.color?.fillAlpha ?? 210;
  const hueMultiplier = options.sketch?.color?.hueMultiplier ?? 3;
  const opacityFactor = options.sketch?.color?.opacityFactor ?? 1.5;
  const noiseScale = options.sketch?.shape?.noiseScale ?? 10;

  alphaPoints.forEach( ( {
    alpha, position
  } ) => {
    const baseDepth = mappers.fn(
      alpha,
      0,
      255,
      0,
    ...
    const noiseInput = p.noise( position.x / columns / noiseScale +
        position.y / rows / noiseScale +
        animation.angle / 2 );
    ...
    const hue = p.noise(
      ...
      position.y / rows / 2 + animation.angle / 2 + d / 150
    );
