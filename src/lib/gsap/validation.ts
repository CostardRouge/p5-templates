...
import { GSAPTemplateOptions, GSAPAnimationOptions } from '@/types/gsap-template.types';
...
export function validateAnimationOptions(
  options: unknown
): options is GSAPAnimationOptions {
  ...
  if (typeof opts.framerate !== 'number' || opts.framerate <= 0) {
    throw new TemplateValidationError(
      'Animation framerate must be a positive number'
    );
  }

  if (opts.duration > 300) {
    throw new TemplateValidationError(
      'Animation duration cannot exceed 300 seconds (5 minutes)'
    );
  }

  if (opts.framerate > 120) {
    throw new TemplateValidationError(
      'Animation framerate cannot exceed 120 fps'
    );
  }
...
export function validateTemplateOptions(
  options: unknown
): options is GSAPTemplateOptions {
  if (!options || typeof options !== 'object') {
 ...
 * Validate RGB color
 */
export function validateRGBColor(value: unknown, fieldName: string): void {
  if (!isRGBColor(value)) {
    throw new TemplateValidationError(
      `${fieldName} must be an RGB array [r, g, b] with values 0-255`
    );
  }
}
...
export function safeParseOptions<T extends GSAPTemplateOptions>(
  ...
  try {
    ...
    throw new TemplateValidationError(
      `Failed to parse options: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
