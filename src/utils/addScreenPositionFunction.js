...
export default async function addScreenPositionFunction( p5Instance ) {
  let p = p5Instance || this;
  const p5Class = await loadP5Class();
  ...
  p.screenPosition = function(
    x, y, z
  ) {
    if ( x instanceof p5Class.Vector ) {
      let v = x;
  ...
  function multMatrixVector(
    m, v
  ) {
    if ( !( m instanceof p5Class.Matrix ) || !( v instanceof p5Class.Vector ) ) {
