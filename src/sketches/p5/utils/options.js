...
// parameters under `settings`, and a photo sketch keeps its image paths in
// there — exactly like the page's own `sketch` block, which is already walked
// below. Without this, dropping a photo sketch in as a layer would render it
// with no photos at all. Only that one item type is walked: every other content
// item resolves its images out of the asset list, which is collected already.
function collectEmbeddedSketchImages(
  content, acc
) {
