...
sketch.draw( (
  ...
  const gridRows = options.sketch.grid?.rows ?? 4;
  ...
  const marginBottom = options.sketch.path?.marginBottom ?? 200;
  ...
  const waveStrengthEasing = easing?.[ options.sketch.waves?.strengthEasing ] ?? easing.easeInOutElastic;
  ...
  const rotationAmplitude = options.sketch.rotation?.amplitude ?? p.PI / 3;
  const rotationStepDivider = options.sketch.rotation?.stepDivider ?? 30;
  const rotationEasing = easing?.[ options.sketch.rotation?.easing ] ?? easing.easeInOutSine;
