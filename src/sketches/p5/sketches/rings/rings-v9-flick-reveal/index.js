...
// through a TRANSITION PLAN (see virtualPointers.js): matched points are
// THROWN to their new positions, missing points are fetched off-canvas by an
// add-pointer cursor and thrown in, surplus points are flicked off-canvas —
// retiring when their flight ends. The live shape is a pool of point slots:
// at every beat boundary the pool SNAPS to the finished word (positions +
// ring links), so the conversion may be as messy as it likes — including
// your own mouse meddling mid-show — and the word still lands exactly.
// Thrown-in points float as lone beads until the snap splices them into the
// outline.
//
// ── The clicks (Click sounds) ────────────────────────────────────────────────
// The pull is a real drag, so the conversion stays audible: drop an audio
// file on "Mouse down" and another on "Mouse up" and every grab / release of
// a point fires them — the two halves of a real click. Because the sound
// layer watches the DRAG layer, not the pointer devices, the scripted
// cursors, the mouse, a finger and a camera pinch all click through the same
// path, and a press over empty canvas stays silent. The flight itself is
// silent — no pointer holds the point once it is in the air. See
...
// v8 unchanged. Nothing persists: the cycle is the artwork.
//
// ── Loop safety ──────────────────────────────────────────────────────────────
...
// uT = TAU meets uT = 0 exactly, live and in deterministic capture. Flights
// are clock-resolved and clamped inside their beat, so every thrown point
// has landed before the snap. Colours close too: each capsule's hue identity
// eases from its current word's arc id to the id it will carry after the
// beat snap (rings-v4's continuous uPipeId, per point), so no boundary —
// including the wrap back to word[0] — recolours the word abruptly.
// ─────────────────────────────────────────────────────────────────────────────
...
const FRAGMENT = `
  ...
  // (a == b) degenerates to a sphere — that is how thrown points render.
...
const troupe = createFlickTroupe();
...
function rebuildPool(
  ...
  transition.adds.slice(
    ...
    // A thrown-in bead wears its destination arc id from the start, so the
...
// ── Depth pinch: thumb + MIDDLE finger = grab in z (v7/v8, unchanged) ───────
...
sketch.draw( () => {
  ...
  const virtualPointers = troupe.update( {
    ...
    flyPoint: (
      ...
      // A thrown point in flight — but the user's grip beats the throw: a
      // slot a real pointer (mouse, finger, pinch) is holding stays theirs.
      for ( const [
        key,
        held
      ] of draggable.drags ) {
        if ( held === i && !key.startsWith( VIRTUAL_POINTER_PREFIX ) ) {
          return;
        }
      }

      // Place the point on its flight path in the view-parallel plane
      // through it (same move as a drag), so the spring tracks the pointer
      // path exactly for any camera yaw/pitch.
      const world = unprojectPoint(
        p,
        rig,
        screenPt,
  ...
  // place at the beat snap instead of being thrown like the others. The
  ...
  // stays silent. The flight after the release is silent — no pointer holds
  // the point once it is in the air.
  clicks.update(
    draggable.drags,
    o.sound
  );
  ...
  // in v9 — the cycle itself is the artwork.
  for ( const key of [
    ...dragCursor.keys()
  ] ) {
