...
// ─────────────────────────────────────────────────────────────────────────────
// Glyph morphing — turn one letter's outline into another's.
//
// Both letters are resampled to a fixed point count so corresponding points can be
// lerped. Outer outlines line up by their topmost point (a cheap, stable
// correspondence) and a normalised winding so they never lerp inside-out; holes
// pair by index, and a hole present on only one side grows from / shrinks to a
// point. The result is a contour list ready for geometryFromContours().
// ─────────────────────────────────────────────────────────────────────────────

// Fixed point counts the morph resamples each glyph's outer outline / holes to.
const N_OUTER = 80;
const N_HOLE = 28;

// char|sampleFactor → { outer, holes } resampled rings. Renderer-free, so it
// survives across graphics instances.
...
export function getMorphSource(
  char, font, sampleFactor
) {
  const key = `${ char }|${ sampleFactor }`;
...
// A hole that exists on only one side morphs to/from its own centroid.
...
// Contours of glyph A morphed toward glyph B at t∈[0,1].
export function morphContours(
  a, b, t
) {
