...
export async function POST(
  ...
  try {
    ...
    await page.goto(
      `http://localhost:3000/p5/${ template }?captureOptions=${ minifyAndEncodeCaptureOptions( captureOptions ) }`,
      {
    ...
    // Wait for download
    const download = await page.waitForEvent(
      "download",
      {
        timeout: 30_000_000
      }
    ...
    const ffmpegOptions = [
      "-r",
      String( captureOptions.animation.framerate ),
      // '-i', downloadPath,
    ...
    return new Response(
      "Video processing failed: " + JSON.stringify( error ),
      {
        status: 500
      }
    );
