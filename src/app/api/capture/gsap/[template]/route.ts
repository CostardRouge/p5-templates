...
import { validateTemplateOptions, TemplateValidationError } from '@/lib/gsap/validation';
...
export async function POST(
  ...
  const framesDir = path.join(tmpDir, `gsap_${template}_${timestamp}`);
  ...
  try {
    ...
    await execAsync(
      `ffmpeg -framerate ${framerate} -i ${framesDir}/frame_%05d.png -c:v libx264 -pix_fmt yuv420p -y ${outputPath}`
    );
