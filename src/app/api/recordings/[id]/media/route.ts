...
import {
  getDownloadUrlFromS3Url, getObjectSize
...
export async function GET(
  ...
  try {
    ...
    const thumbnailSignedUrls = await Promise.all( thumbnails.map( async( key ) => {
      try {
        return await getDownloadUrlFromS3Url(
          key,
          3600
        );
      } catch( error ) {
    ...
    const videoData = await Promise.all( videoUrls.map( async(
      key, index
    ) => {
      try {
        const url = await getDownloadUrlFromS3Url(
          key,
          3600
        );
    ...
    if ( validVideos.length > 0 ) {
      const totalVideoSize = validVideos.reduce(
        (
          sum, video
        ) => sum + ( video.size || 0 ),
        0
