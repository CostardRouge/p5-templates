...
export async function POST(
  ...
  try {
    ...
    if (["completed", "failed", "cancelled"].includes(dbJob.status)) {
      return NextResponse.json({ cancelled: false, reason: "already finalized" });
    }
    ...
    if (bullJob) {
      ...
      if (state === "active") {
        return NextResponse.json({ cancelled: false, reason: "job is active" });
      }
    }

    // No bull job: if it's draft or other pre-flight, don't mark cancelled, suggest delete
    return NextResponse.json({ cancelled: false, reason: "not cancellable in current state" });
  } catch (error) {
    console.error(`[POST /api/recordings/${jobId}/cancel]`, error);
