...
export async function GET(
  ...
  try {
    ...
    if (job.status === "completed" && job.thumbnails) {
      ...
      if (thumbnails.length > 0) {
        try {
          ...
          const signedUrl = await getDownloadUrlFromS3Url(thumbnails[0], 3600);
    ...
    const templateThumbnailUrl = getP5SketchThumbnailURL(job.template.replace("p5", ""));
    
    // Convert relative URL to absolute URL for redirect
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 
                    `${_req.nextUrl.protocol}//${_req.nextUrl.host}`;
    const absoluteUrl = new URL(templateThumbnailUrl, baseUrl).toString();
    
    return NextResponse.redirect(absoluteUrl, 302);
  } catch (error) {
    console.error(`[GET /api/recordings/${jobId}/thumbnail]`, error);
