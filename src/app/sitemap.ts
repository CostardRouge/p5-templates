...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    {
      url: `${baseUrl}/templates/gsap`,
      lastModified: now,
      changeFrequency: "weekly",
      priority: 0.8,
    },
    {
      url: `${baseUrl}/templates/html/exif-detail`,
      lastModified: now,
  ...
  const sketchPages: MetadataRoute.Sitemap = sketches.map((sketch) => ({
    url: `${baseUrl}/templates/p5/${sketch.name}`,
    lastModified: now,
  ...
  const gsapPages: MetadataRoute.Sitemap = gsapTemplates.map((template) => ({
    url: `${baseUrl}/templates/gsap/${template.id}`,
    lastModified: now,
    changeFrequency: "monthly" as const,
    priority: 0.8,
  }));

  return [...staticPages, ...sketchPages, ...gsapPages];
}
