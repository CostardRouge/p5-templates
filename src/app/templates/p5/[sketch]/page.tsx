...
import { getBaseUrl, getBreadcrumbJsonLd, getTemplateJsonLd } from "@/lib/seo";
...
async function ProcessingSketch({
  ...
  const { formValues, formConfiguration } = await getSketchMeta(sketchName);
  ...
  const templateJsonLd = getTemplateJsonLd({
    ...
    url: `${baseUrl}/templates/p5/${sketchName}`,
