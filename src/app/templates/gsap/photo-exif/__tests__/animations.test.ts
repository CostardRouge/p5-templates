...
import { calculateTotalFrames, calculateProgress, dispatchSeekFrame, setupFrameSync } from '@/lib/gsap/syncWithRecording';
import type { PhotoExifTemplateOptions } from '../types';
import { defaultOptions } from '../options';

describe('Photo EXIF Template Animations', () => {
  beforeEach(() => {
  ...
  describe('Animation timing at different framerates', () => {
    it('should complete animation at correct frame count for 24fps', () => {
    ...
    it('should complete animation at correct frame count for 30fps', () => {
      const options: PhotoExifTemplateOptions = {
        ...defaultOptions,
        animation: {
          duration: 5,
    ...
    it('should complete animation at correct frame count for 60fps', () => {
      const options: PhotoExifTemplateOptions = {
        ...defaultOptions,
        animation: {
          duration: 5,
  ...
  describe('Timeline duration synchronization', () => {
    ...
    it('should maintain duration after adding animations', () => {
      const duration = 5;
      const timeline = gsap.timeline({ paused: true });
    ...
    it('should sync progress correctly across different durations', () => {
  ...
  describe('Animation phase timing', () => {
    it('should calculate correct phase durations', () => {
      const duration = 10;
    ...
    it('should ensure phases fit within total duration', () => {
      const duration = 5;
  ...
  describe('GSAP animation properties', () => {
    it('should set initial opacity to 0 for image', () => {
      ...
      const computedOpacity = gsap.getProperty('#image', 'opacity');
      const computedScale = gsap.getProperty('#image', 'scale');
    ...
    it('should animate image to full opacity and scale', () => {
      const mockElement = document.createElement('div');
      mockElement.id = 'image';
      document.body.appendChild(mockElement);
      ...
      const finalOpacity = gsap.getProperty('#image', 'opacity');
      const finalScale = gsap.getProperty('#image', 'scale');
   ...
   */
  describe('Property 3: Timeline Progress Monotonicity', () => {
    it('should maintain monotonically increasing progress for any animation configuration', () => {
      fc.assert(
        fc.property(
          ...
          (animationOptions) => {
            ...
            for (let frame = 0; frame < totalFrames; frame++) {
              const progress = calculateProgress(frame, totalFrames);
    ...
    it('should maintain monotonicity with frame synchronization events', () => {
      fc.assert(
        fc.property(
          fc.record({
          ...
          (animationOptions) => {
            ...
            const cleanup = setupFrameSync(timeline, animationOptions, (frame, progress) => {
    ...
    it('should handle edge case of single frame animation', () => {
      fc.assert(
        fc.property(
          fc.record({
            duration: fc.float({ min: 0.01, max: 0.1, noNaN: true }),
            framerate: fc.constantFrom(24, 30),
          }),
          (animationOptions) => {
            const timeline = gsap.timeline({ paused: true });
            timeline.duration(animationOptions.duration);
