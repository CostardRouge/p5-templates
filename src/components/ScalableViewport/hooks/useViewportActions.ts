...
export function useViewportActions({
  ...
  const fitToViewport = useCallback(
    (animate = false) => {
      ...
      const { x, y, scale } = calculateFitToViewport(
        contentWidth,
        contentHeight,
        viewportWidth,
        viewportHeight
      ...
      if (animate) {
        ...
        setTransform({ x, y, scale }, contentRef.current);
      }
    },
    [containerRef, contentRef, setTransform, animateTo]
  );
  ...
  const resetToActualPixels = useCallback(
    (animate = false) => {
      ...
      const { x, y, scale } = calculateActualPixels(
        contentWidth,
        contentHeight,
        viewportWidth,
        viewportHeight
  ...
  }, [containerRef, transform, animateTo]);
