 ...
 */
export default function useFormUndoRedoHistory<T = any>() {
  const context = useFormUndoRedo<T>();
  ...
  const getHistoryItems = () => {
    return {
      past: history.past.map((entry, index) => ({
        index,
      ...
      future: history.future.map((entry, index) => ({
        index,
