...
import ItemPalette from "../../ContentItems/components/AddItemControls/components/ItemPalette/ItemPalette";
import type {
  ItemKind
 ...
 * that unfolds the type palette right under that header.
 *
 ...
 * new layer would land. With no slides there is only one group, so it reads as
 * the single "+ Add" it used to be.
 */
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* The palette animates open under the header rather than appearing at
          ...
          eleven tiles cost nothing on a panel nobody adds to. Its own header
          is empty and hidden — the group's `+` is the control. */}
      <CollapsibleItem
        expanded={ paletteOpen }
        onToggle={ setPaletteOpen }
        header={ () => <></> }
        headerContainerClassName="hidden"
        contentClassName="pb-1"
      >
        <ItemPalette onAdd={ handleAdd } />
      </CollapsibleItem>

      {fields.length === 0 && !paletteOpen && (
