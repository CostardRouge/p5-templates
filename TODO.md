...
- [ ] **MIDI learn as a gesture** — every CC a controller sends is now its own channel and the picker lists them as they arrive, so a knob is bound by choosing `CC 113` from the list. What is left is the gesture: arm, wiggle, assigned. The mechanism is not `midi.ccLast` (it carries the value, never the number) but a frame-to-frame diff of the channel snapshot, taking the scalar that moves most — which makes it work for a fader, an audio band or a hand gesture too. Two ordering traps and the reason they bite are in `docs/memory/interaction-bindings.md`
