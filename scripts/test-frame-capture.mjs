...
async function captureFramesServerSide(
  page, framesDirectory, totalFrames
) {
  await fs.mkdir(
    framesDirectory,
    {
...
async function runTest() {
  ...
  try {
    ...
    // Navigate to sketch
    log(
      `\n🌐 Loading sketch: ${ TEST_CONFIG.url }`,
      colors.blue
    );
    await page.goto(
      TEST_CONFIG.url,
    ...
    if ( firstFrame === "frame_00000.png" && lastFrame === `frame_${ String( TEST_CONFIG.totalFrames - 1 ).padStart(
      5,
      "0"
    ) }.png` ) {
      log(
      ...
      log(
        "   ⚠️  Frame naming may be incorrect",
        colors.yellow
      );
    }
    ...
    log(
      `\n📁 Test frames saved to: ${ TEST_CONFIG.outputDir }`,
      colors.blue
    );
    log(
      `   To view frames: open ${ TEST_CONFIG.outputDir }`,
      colors.blue
    );
    log(
