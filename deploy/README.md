...
|---|-----------------------------------------------------------------------------------|---|
| **Cloudflare Tunnel** | `cloudflared` on the NAS → `https://watchtower.steeve.website` → `localhost:8080` | None |
...
2. Add a **Public Hostname**: `watchtower.steeve.website` → service
...
`https://watchtower.steeve.website/v1/update?image=ghcr.io/costardrouge/p5-templates`.
...
| Secret | Value |
|---|---|
| `DEPLOY_WEBHOOK_URL` | `https://watchtower.steeve.website/v1/update?image=ghcr.io/costardrouge/p5-templates` |
...
the redeployment step is skipped (so nothing breaks while you wire this up).
