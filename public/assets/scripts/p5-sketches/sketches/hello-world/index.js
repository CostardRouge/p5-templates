...
sketch.draw( (time, center, favoriteColor) => {
    ...
    string.write(options.texts.top || "top", width*.1, height*.2, {
        size: 92,
    ...
    string.write(options.texts.bottom || "bottom", width*.9, height*.8, {
        textAlign: [RIGHT],
        size: 92,
        stroke: color(...options.colors.background),
        fill: color(...options.colors.text, 190),
    ...
    graphic.push()
    graphic.translate(w, h);
    graphic.circle(
        mappers.fn(animation.sinOscillation, -1, 1, -w+m, w-m),
        mappers.fn(animation.cosOscillation, -1, 1, -h+m, h-m),
        90
    )
    graphic.pop()
