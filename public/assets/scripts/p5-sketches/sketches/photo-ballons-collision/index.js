...
function addImageBall(img, x, y, radius) {
	...
	const newImageBall = {
		img,
		ball: Bodies.circle(x, y, radius, radius)
	}
	...
	Composite.add(matter.engine.world, newImageBall.ball)
}
...
sketch.draw( ( time, center, favoriteColor ) => {
	...
	matter.balls.forEach( ({ img, ball }, index) => {
		const { position: { x, y }, circleRadius } = ball;

		matter.balls.forEach( ({ img, ball }, _index) => {
			if (index == _index) {
				return;
			}
		...
		drawImageWithMask({
			...
			maskDrawer: graphics => {
				graphics.fill(255);
				graphics.noStroke();
				graphics.ellipse(x, y, circleRadius*2, circleRadius*2)
			}
		})
	})
