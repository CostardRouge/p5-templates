import { sketch, events, mappers, animation, imageUtils, string, captureOptions as options, shapes } from '/assets/scripts/p5-sketches/utils/index.js';
...
sketch.setup(
	() => {
		...
		_ml5.capture.element = createCapture(VIDEO, { flipped: true } );
		_ml5.capture.element.size(_ml5.capture.size.width, _ml5.capture.size.height);
		_ml5.capture.element.hide();

		_ml5.webcam.element = createCapture(VIDEO, { flipped: true } );
		_ml5.webcam.element.size(_ml5.webcam.size.width, _ml5.webcam.size.height);
		...
		_ml5.bodySegmentation.detectStart(_ml5.capture.element, results => {
...
function drawHand(hand, color, graphics = window) {
	...
	hand.forEach( tip => {
		graphics.vertex(
			map(tip.x, 0, _ml5.capture.size.width, 0, _ml5.webcam.size.width),
			map(tip.y, 0, _ml5.capture.size.height, 0, _ml5.webcam.size.height),
		);
	})
...
sketch.draw( (time, center, favoriteColor) => {
	background(...options.colors.background, 10);
	...
	if (_ml5.segmentation) {
		...
		translate(width, 0);
		scale(-1, 1);
		image(_ml5.segmentation.mask, 0, 0, width, height);
		pop();
	}
	...
	string.write(options.name, width/2, height*.2, {
		size: 92,
