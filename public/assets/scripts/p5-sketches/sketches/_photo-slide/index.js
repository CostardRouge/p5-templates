...
function drawImageWithMask({
	...
	canvases.maskBuffer.rect(0, 0, canvases.maskBuffer.width, canvases.maskBuffer.height);
	canvases.maskBuffer.noErase();

	canvases.imageBuffer.erase();
	canvases.imageBuffer.rect(0, 0, canvases.imageBuffer.width, canvases.imageBuffer.height);
...
sketch.draw( ( time, center, favoriteColor ) => {
	...
	const imageIndices = imageObjects.map( (_, index) => index );
	...
	const currentImageIndex = mappers.circularIndex(animation.progression*imageIndices.length, imageIndices)
	const nextImageIndex = mappers.circularIndex(1+animation.progression*imageIndices.length, imageIndices)
	...
	canvases.sliderBuffer.rect(0, 0, canvases.sliderBuffer.width, canvases.sliderBuffer.height);
	...
	imageUtils.marginImage({
		...
		position: createVector(offset, height/2),
	})
	...
	imageUtils.marginImage({
		...
		center: true,
		graphics: canvases.sliderBuffer,
		position: createVector(offset+width+width/2, height/2),
	});
