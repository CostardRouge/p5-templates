...
const mappers = {
  ...
  circularConstrain: function(
    ...
    if ( value > max ) {
      return mappers.circularConstrain(
        min + rest,
        min,
        max
      );
  ...
  traceVectors(
    count, vectorsGenerator, onStart, onTrace, onEnd, smoothLength = false, smoothSteps = false, smoothStepsCount = 1
  ) {
    const vectorsList = Array( count ).fill( undefined )
      .map( (
        _, index
      ) => (
