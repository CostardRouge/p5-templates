...
const getDefaultOptions = () => {
  ...
  defaultOptions.push( ...[
    ...
    },
    {
      ...
      defaultValue: true,
      category: "Canvas",
      onChange: checked => events.handle(
        "engine-smooth-pixel-change",
        checked
      )
    },
    {
  ...
  defaultOptions.push( {
    ...
    onChange: value => {
      const [
        canvasWidth,
        canvasHeight
      ] = sketch.getDefaultCanvasSize( value );
...
const options = {
  ...
  create: (
    option, synchronize = false
  ) => {
    options.addedOptions.push( option );

    const existingOptionIds = options.addedOptions.map( ( {
      id
    } ) => id );
